...
- 모든 Token은 UUID를 가짐
- 모든 로그인은 최초 발생 시 UUID로 세션 ID를 가지며, 이는 Refresh 토큰 또한 가짐.
- 모든 Access Token은 발급 때마다 고유한 UUID를 가지며, 세션ID 또한 가지고 있음
...
- Redis 에는 세션 ID, Access Token의 UUID, TTL 세 가지 정보를 저장함
- Access Token을 재발급할 떄마다, Refresh Token도 갱신하며, Redis 에서의 UUID와 TTL을 갱신함

#### Access Token 인증 시
- Token의 기본 Validation을 진행
- Valid하다면, Access Token이 가지고 있는 Session ID와 UUID를 확인함
- Session ID를 통해 Key-Value pair를 조회함
- 존재하고, Value(UUID)가 동일한지 확인함
- 동일하면 OK
...
- 새로운 UUID를 발급함
- Access Key와 Refresh Key를 발급함
- Redis의 TTL과 Value(UUID)를 갱신함
...
- UUID A 인 Fresh한 Refresh Token 을 사용자가 들고 있음
  - Redis에는 A 가 저장되어 있음
- 부정 사용자가 A 토큰을 탈취하여, UUID B를 가진 Refresh/Access Token pair를 발급함
  - Redis에서는 Key가 Session ID인 Value의 A가 B로 갱신됨
- 기존 사용자가 A 토큰을 사용해 Access Token 발급을 요청함
  - Redis에 A 정보가 아니므로, UUID가 A인 토큰이 부정 사용되었음을 알 수 있음
  - 해당 Session ID가 Key인 Redis의 Key-Value 데이터를 삭제함


#### 로그아웃 시
- Session ID KEY 데이터를 삭제함